...
//void createRandomHairyMst(int numVertices, int radius, VertexVector** vertices, EdgeVector** edges) {
...
//
//	// CDT -> EdgeVector
//	EdgeVector* cdtEdges = new EdgeVector();
...
//		cdtEdges->push_back(edge);
//	}
//
//	(*edges) = computeCustomMst((*vertices), cdtEdges);
//
...
//	}
//
//	// Add all edges from mst to disjoint set
//	for (int i = 0; i < (*edges)->size(); i++) {
//		SimpleEdge* edge = (**edges)[i];
//		ds.link(edge->u, edge->v);
//	}
//
//	// Add cdt edges to mst edges if they do not form cycles
//	for (int i = 0; i < cdtEdges->size(); i++) {
//		SimpleEdge* edge = (*cdtEdges)[i];
